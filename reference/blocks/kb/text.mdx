---
title: Text KnowledgeBase
sidebarTitle: Text
---

## Example

<Note>

Install `textract` if needed using [this guide](/how-to/install)

</Note>

```python knowledge_base.py
from phi.knowledge.text import TextKnowledgeBase
from phi.vectordb.pgvector import PgVector2

from resources import vector_db

knowledge_base = TextKnowledgeBase(
    path="data/docs",
    # Table name: ai.text_documents
    vector_db=PgVector2(
        collection="text_documents",
        db_url=vector_db.get_db_connection_local(),
    ),
)
```

## TextKnowledgeBase Params

<ResponseField name="path" type="Union[str, Path]">
  Path to text files. Can point to a single docx file or a directory of docx
  files.
</ResponseField>
<ResponseField name="formats" type="List[str]">
  Formats accepted by this knowledge base. Default: `[".txt"]`
</ResponseField>
<ResponseField name="reader" type="TextReader">
  A `TextReader` that converts the text files into `Documents` for the vector
  database.
</ResponseField>

## AssistantKnowledge Params

`TextKnowledgeBase` is a subclass of the `AssistantKnowledge` class and has access to the same params

<Snippet file="kb-base-reference.mdx" />
