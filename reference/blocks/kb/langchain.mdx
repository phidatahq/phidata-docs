...
conv = Conversation(knowledge_base=knowledge_base, add_references_to_prompt=True)
conv.print_response("What did the president say about technology?")
```

## LangChainKnowledgeBase Reference

<ResponseField name="retriever" type="Any" default="None">
  LangChain retriever.
</ResponseField>
<ResponseField name="vectorstore" type="Any" default="None">
  LangChain vector store used to create a retriever.
</ResponseField>
<ResponseField name="search_kwargs" type="dict" default="None">
  Search kwargs when creating a retriever using the langchain vector store.
</ResponseField>

## KnowledgeBase Reference

`LangChainKnowledgeBase` is a subclass of the `KnowledgeBase` class and has access to the same params
