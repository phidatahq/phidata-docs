...
<ResponseField name="loader" type="Callable">
  LangChain retriever.
</ResponseField>
<ResponseField name="vectorstore" type="Any">
  LangChain vector store used to create a retriever.
</ResponseField>
<ResponseField name="search_kwargs" type="dict" >
  Search kwargs when creating a retriever using the langchain vector store.
</ResponseField>
<ResponseField name="retriever" type="Any">
  LangChain retriever.
</ResponseField>

## AssistantKnowledge Reference

`LangChainKnowledgeBase` is a subclass of the `AssistantKnowledge` class and has access to the same params
