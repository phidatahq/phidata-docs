...
sidebarTitle: "Quickstart: RAG LLM App"
description: "Let's build a RAG LLM App powered by GPT-4. We'll use PgVector for Knowledge Base and Storage and serve the app using Streamlit and FastApi."
---
...
Create your codebase using the `llm-app` template pre-configured with [FastApi](https://fastapi.tiangolo.com/), [Streamlit](https://streamlit.io/) and [PgVector](https://github.com/pgvector/pgvector). Use this codebase as the starting point for your RAG LLM App.
...
The `pdf_knowledge_base` in the `llm/knowledge_base.py` file provides the references (context) used for RAG. The `PDFKnowledgeBase` reads local PDFs and the `PDFUrlKnowledgeBase` reads PDFs from URLs.
...
## Build your RAG LLM Product

Your LLM Api provides common endpoints you can use to build your LLM product. These routes are developed in collaboration with real LLM Apps and are a great starting point to build on.
...
- [Update the dev application](how-to/dev-app)
- [Update the production application](how-to/production-app)
- [Format and validate your code](how-to/format-and-validate)
- [Add python libraries](how-to/python-libraries)
- [Add a custom domain and HTTPS](how-to/domain-https)
- [Implement CI/CD](how-to/ci-cd)
