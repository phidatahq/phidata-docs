---
title: Session State
description: "**Use `Workflows.session_state` to cache results in a database.**"
---

All Workflows come with a `session_state` dictionary that can be used to cache results. Provide your workflows with `storage` and a `session_id` to enable caching.

For example, you can use the `SqlWorkflowStorage` to cache results in a Sqllit database.
