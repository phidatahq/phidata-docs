...
**Assistants** make LLMs smarter with extensions like memory, knowledge and tools.

A simple assistant (without extensions) looks like:
...
Under the hood, the description and instructions are converted into the system prompt and the input is passed as the user prompt.

`debug_mode=True` prints the prompts sent to the LLM, here's how they look like:
