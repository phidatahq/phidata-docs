---
title: What is phidata?
sidebarTitle: Introduction
mode: "wide"
---

**Phidata is a framework for building AI Assistants with memory, knowledge and tools.**

## Why use phidata

**Problem:** LLMs have limited business context and cannot take actions.<br />
**Solution:** Add memory, knowledge and tools to turn them into AI Assistants.
- **Memory:** Enables LLMs to have long-term conversations by storing chat history in a database.
- **Knowledge:** Provides LLMs with relevant business context by storing information in a vector database.
- **Tools:** Enable LLMs to take actions like pulling data from an API, sending emails or querying a database.

**Memory** & **knowledge** make LLMs smarter while **tools** make them Autonomous. <br />
Together they enable us to build useful AI Assistants that can achieve any task.

![assistant_intro](/images/assistant_intro.png)

## How it works

- **Step 1:** Create an `Assistant`
- **Step 2:** Add Tools (functions), Knowledge (vectordb) and Storage (database)
- **Step 3:** Serve using Streamlit, FastApi or Django to build your AI application

<p className="hidden">

LLM = Large Language Model

</p>

## Example

<Steps>
  <Step title="Install phidata">

    ```bash
    pip install -U phidata
    ```

  </Step>
  <Step title="Create an Assistant">

    Create a file `assistant.py` with an Assistant that can search the web using DuckDuckGo.

    ```python assistant.py
    from phi.assistant import Assistant
    from phi.tools.duckduckgo import DuckDuckGo

    assistant = Assistant(tools=[DuckDuckGo()], show_tool_calls=True)
    assistant.print_response("Whats happening in France?", markdown=True)
    ```

  </Step>
  <Step title="Run the Assistant">

    Set your `OPENAI_API_KEY` (you can get one from [here](https://platform.openai.com/account/api-keys))

    <CodeGroup>

    ```bash Mac
    export OPENAI_API_KEY=sk-***
    ```

    ```bash Windows
    setx OPENAI_API_KEY sk-***
    ```

    </CodeGroup>

    Install `openai` & `duckduckgo`

    ```shell
    pip install openai duckduckgo-search
    ```

    Run the Assistant

    ```shell
    python assistant.py
    ```

  </Step>
</Steps>

## Demos

Checkout the following AI Applications built using phidata:

- [PDF AI](https://pdf.aidev.run/) summarizes and answers questions from PDFs.
- [ArXiv AI](https://arxiv.aidev.run/) answers questions about ArXiv papers using the ArXiv API.
- [HackerNews AI](https://hn.aidev.run/) summarize stories, users and shares what's new on HackerNews.

## Next Steps

1. Read the [basics](/basics) to learn more about phidata.
2. Read about [Assistants](/assistants/introduction) and how to customize them.
3. Checkout the [cookbook](/examples/cookbook) for in-depth examples and code.

## Building AI for your product?

We've helped many companies build AI for their products, the general workflow is:

1. **Build an Assistant** with proprietary data to perform tasks specific to your product.
2. **Connect your product** to the Assistant via an API.
3. **Customize, Monitor and Improve** your AI product.

We also provide dedicated support and development, [book a call](https://cal.com/phidata/intro) for more information.
