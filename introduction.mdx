...
**Phidata is a toolkit for building AI applications using LLM function calling.** It provides pre-built templates for AI apps that you can clone & run in minutes. Here's how it works:

- Create your AI App using a template: `phi ws create`
- Run your app locally: `phi ws up`
- Deploy your app to AWS: `phi ws up prd:aws`

<p className="hidden">"LLM = Large Language Model"</p>

## Under the hood

Phidata provides **Assistants** that achieve tasks by letting LLMs call functions and take actions.

This is a powerful approach that allows LLMs to solve complex problems by calling functions and intelligently choosing their course of action based on the response.

For example, to answer a question from a proprietary knowlege base, an Assistant will first call a function to search the knowlege base and then systhesize the results to return the answer.
