...
| `formats`       | `List[str]`        | `[".csv"]` | Formats accepted by this knowledge base.                                            |
| `vector_db`     | `VectorDb`         | -          | Vector Database for the Knowledge Base.                                             |
| `reader`        | `CSVReader`        | -          | A `CSVReader` that converts the CSV files into `Documents` for the vector database. |
