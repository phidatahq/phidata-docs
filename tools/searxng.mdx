...
| `_search` | Internal function to handle the search logic for all categories. Parameters include `query` for the search term, `category` to specify the type of content, and `max_results` for the maximum number of results (default is 5). Returns the search results or an error message. |
