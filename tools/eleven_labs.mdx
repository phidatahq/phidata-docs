...
**ElevenLabsTools** enable an Agent to perform audio generation tasks.
...
The following agent will use Eleven Labs to generate a audio based on a user prompt.
...
| `target_directory` | `str` | `None` | The directory to save the audio file |
| `model_id` | `str` | `eleven_multilingual_v2` | The model's id to use for the audio generation |
| `output_format` | `str` | `mp3_44100_64` | The output format to use for the audio generation |
...
| `_process_audio` | Convert the audio to base64 to send to Frontend and optionally save to target directory |


## Information

- View on [Github](https://github.com/phidatahq/phidata/blob/main/phi/tools/eleven_labs_tools.py)
