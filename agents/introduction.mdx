...
Use `debug_mode=True` to log the raw messages used by the agent.
</Note>
...
The `Agent.run()` function returns a `RunResponse` object or an stream of `RunResponse` objects. `RunResponse` contains the following attributes:

TODO: @yash to update

<ResponseField name="content" type="str" default="None">
