...
The `streamlit-app` provides pre-configured codebase with [Streamlit](https://streamlit.io/) & [PostgreSQL](https://www.postgresql.org/), a stack we love for its simplicity. Its simple but not to be underestimated.
...
A common use case is to just use Streamlit without the database. In that case, update the `workspace/settings.py` file and set `dev_db_enabled=False`.
