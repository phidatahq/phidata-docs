...
Tools are functions that the Assistant can run to achieve tasks. They enable LLMs to "take an action" and choose their path based on the response, solving problems in a human-like manner.

For example, to build a text-to-SQL assistant, we can provide tools to view and describe tables, search the knowledge base for context and finally, inspect and run queries to get the answer. This allows the text-to-SQL assistant to work through the problem in an intelligent manner.
...
auto_assistant.print_response('How do I make chicken tikka salad?')
...
<ResponseField name="update_knowledge_base" type="bool" default="False">
  If use_tools is True and update_knowledge_base is True, then `add_to_knowledge_base` tool is added that allows the LLM to update the knowledge base.
</ResponseField>
