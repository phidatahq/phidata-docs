...
In general, Conversations follow the `Input -> Output` pattern and Tasks allows us to achieve `Input -> Output 1 -> ... -> Final Output` by chaining the LLM outputs together.

## Usage

In the following example, we use 3 `Tasks` to reliably generate short stories for a given `Theme` by:
...
<Steps>
  ...
  <Step title="Create a conversation with tasks">
