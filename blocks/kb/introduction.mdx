...
To enable Autonomous mode, we add a Knowledge Base to the Assistant and set `tool_calls=True`. This adds a function for searching the **Knowledge Base** and lets the LLM decide when its needs to access the knowledge base and what search parameters to use.
...
auto_assistant = Assistant(
    ...
    # -*- Let the assistant call tools to achieve tasks
    tool_calls=True,
