...
Behind the scenes, we provide the `Assistant` with a set of tools the LLM can call to achieve tasks. Make the Assistant Autonomous by setting `use_tools=True` - which gives it the ability to search the knowledge base and chat history.
