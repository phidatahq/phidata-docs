...
Behind the scenes, we provide the `Assistant` with a set of tools the LLM can call to achieve tasks. Make the Assistant Autonomous by setting `tool_calls=True` - which gives it the ability to search the knowledge base and chat history.
...
<ResponseField name="tool_calls" type="bool" default="False">
  Makes the assistant `Autonomous` by letting the LLM call tools to achieve
  tasks.
</ResponseField>
