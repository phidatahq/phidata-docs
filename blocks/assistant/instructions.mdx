...
The `Assistant` class provides a user-friendly interface to language models but behind the scenes it converts the `description`, `instructions` and `messages` into `system` and `user` prompts.

**Use the following params to customize the system and user prompts:**
...
By default, the Assistant creates a default system prompt using the `description`, `instructions`, `extra_instructions`, `markdown` and `output_model` params.
