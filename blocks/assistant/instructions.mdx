...
When you add a `description`, it's added to the beginning of the system prompt. Instructions are added as a list inside `<instructions>` tags which helps the LLM follow them in order.

The default system prompt for the Assistant can be customized using:

<ResponseField name="build_default_system_prompt" type="bool" default="True">
  If True, The Assistant builds a default system prompt using the settings below.
</ResponseField>
...
<ResponseField name="add_knowledge_base_instructions" type="bool" default="True">
  Add instructions for using the knowledge base to the default system prompt if knowledge base is provided
</ResponseField>
<ResponseField name="prevent_hallucinations" type="bool" default="False">
  Add instructions for preventing hallucinations
</ResponseField>
<ResponseField name="prevent_prompt_injection" type="bool" default="False">
  Add instructions to prevent prompt injection attacks
</ResponseField>
<ResponseField name="limit_tool_access" type="bool" default="True">
  Add instructions for limiting tool access to the default system prompt if tools are provided
</ResponseField>
<ResponseField name="add_datetime_to_instructions" type="bool" default="False">
  Add the current datetime to the prompt to give the assistant a sense of time. This allows for relative times like "tomorrow" to be used in the prompt.
</ResponseField>
...
In most cases, the user prompt is simply the `message` sent to the Assistant.
