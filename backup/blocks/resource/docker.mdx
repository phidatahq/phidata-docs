...
[Docker](https://docs.docker.com/get-started/overview/) is a game-changing technology that enables us to run applications locally. We package our [Apps](/blocks/app) into **Containers** that include everything needed to run the application

For example: to run our LLM Api built using `FastApi`, we just need to pull the image and run -- without having to worry about installing libraries, setup or commands.

## Docker Resources

Phidata enables us to define docker resources as pydantic objects so we can build our application layer purely in python. In most cases you will not be creating the **Docker Resources** directly, instead we'll use [Apps](/blocks/app) to create the resources for us.
...
- Package multiple resources into [Apps](/blocks/app) so we can define **"Applications as Code"**.
