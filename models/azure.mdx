---
title: Azure
---

Use the best in class GPT models using Azure's OpenAI API.

## Authentication

Set your environment variables.

<CodeGroup>

```bash Mac
export AZURE_OPENAI_API_KEY=***
export AZURE_OPENAI_ENDPOINT=***
export AZURE_OPENAI_MODEL_NAME=***
export AZURE_OPENAI_DEPLOYMENT=***
# Optional:
# export AZURE_OPENAI_API_VERSION=***
```

```bash Windows
setx AZURE_OPENAI_API_KEY ***
setx AZURE_OPENAI_ENDPOINT ***
setx AZURE_OPENAI_MODEL_NAME ***
setx AZURE_OPENAI_DEPLOYMENT ***
# Optional:
# setx AZURE_OPENAI_API_VERSION ***
```

</CodeGroup>

## Example

Use `AzureOpenAIChat` with your `Agent`:

<CodeGroup>

```python agent.py
import os
from typing import Iterator

from phi.agent import Agent, RunResponse
from phi.model.azure import AzureOpenAIChat

azure_model = AzureOpenAIChat(
    id=os.getenv("AZURE_OPENAI_MODEL_NAME"),
    api_key=os.getenv("AZURE_OPENAI_API_KEY"),
    azure_endpoint=os.getenv("AZURE_OPENAI_ENDPOINT"),
    azure_deployment=os.getenv("AZURE_OPENAI_DEPLOYMENT"),
)

agent = Agent(
    model=azure_model,
)

# Get the response in a variable
# run: RunResponse = agent.run("Share a 2 sentence horror story.")
# print(run.content)

# Print the response on the terminal
agent.print_response("Share a 2 sentence horror story.")
```

</CodeGroup>

## Params

<ResponseField name="id" type="str" required="true">
  The specific model ID used for generating responses. This field is required.
</ResponseField>
<ResponseField name="name" type="str" default="AzureOpenAIChat">
  The name identifier for the agent.
</ResponseField>
<ResponseField name="provider" type="str" default="Azure">
  The provider of the model.
</ResponseField>
<ResponseField name="api_key" type="Optional[str]">
  The API key for authenticating requests to the Azure OpenAI service.
</ResponseField>
<ResponseField name="api_version" type="str" default="2024-02-01">
  The version of the Azure OpenAI API to use.
</ResponseField>
<ResponseField name="azure_endpoint" type="Optional[str]">
  The endpoint URL for the Azure OpenAI service.
</ResponseField>
<ResponseField name="azure_deployment" type="Optional[str]">
  The deployment name or ID in Azure.
</ResponseField>
<ResponseField name="base_url" type="Optional[str]">
  The base URL for making API requests to the Azure OpenAI service.
</ResponseField>
<ResponseField name="azure_ad_token" type="Optional[str]">
  The Azure Active Directory token for authenticating requests.
</ResponseField>
<ResponseField name="azure_ad_token_provider" type="Optional[Any]">
  The provider for obtaining Azure Active Directory tokens.
</ResponseField>
<ResponseField name="organization" type="Optional[str]">
  The organization associated with the API requests.
</ResponseField>
<ResponseField name="openai_client" type="Optional[AzureOpenAIClient]">
  An instance of AzureOpenAIClient provided for making API requests.
</ResponseField>
