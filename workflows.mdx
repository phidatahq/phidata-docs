...
Workflows are deterministic, stateful, multi-agent systems that power many of our production use cases. They are incredibly powerful and offer the following benefits:

- **Control and Flexibility**: We have full control over the multi-agent process, how the input is processed, which agents are used and in what order.
- **Built-in Memory**: We can store state and cache results in a database, meaning our agents can re-use previous steps.
- **Defined as a python class**: We do not need to learn a new framework, its just python.
