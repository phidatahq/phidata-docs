...
**Assistants** solve problems by using LLMs to call functions and take actions. They have their own memory, knowledge base and storage making them ideal for building AI products.
...
Make the `Assistant` Autonomous by setting `use_tools=True` - which gives it the ability to search the knowledge base and chat history.
