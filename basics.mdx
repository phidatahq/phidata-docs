...
Let's learn the basics by building an AI Assistant with a knowledge base, storage and memory.

## Assistants

With phidata, we build AI products using **Assistants**.

**Assistants** provide a human-like interface to language models. They come with built-in **Memory**, **Knowledge**, **Storage** and **Tools**, making it easy to build any kind of AI product.
...
Knowledge Base is a database of information that the Assistant can search to improve its responses. This information provide extra context to language models so they respond in a context-aware manner.
...
Let's test out RAG by building a **PDF Assistant** that helps us read a recipe book.
...
With Autonomous assistants, we let the assistant decide **if** it needs to access the knowledge base and what search parameters it needs to query the knowledge base.

Make the `Assistant` Autonomous by setting `tool_calls=True` - which gives it the ability to search the knowledge base and chat history.
...
<Steps>
  ...
  <Step title="Run the assistant">
    Run the `auto_assistant.py` file and notice how it searches the knowledge base and chat history when needed.
