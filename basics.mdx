...
Retrieval Augmented Generation means providing the LLM with additional information so it responds in a context-aware manner. When a user sends a message, we **"stuff the prompt"** with relevant information from the knowledge base.
...
2. Every time we ask a question, our **PDF Assistant** will respond with the recipes from our knowledge base.
