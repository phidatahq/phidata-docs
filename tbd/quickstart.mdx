...
This command creates a folder `llm-app` with a production-ready RAG LLM App that you can customize to your use-case. It has the following structure:
...
Checkout the `pdf_knowledge_base` in the `llm/knowledge_base.py` for more information. The `PDFKnowledgeBase` reads local PDFs and the `PDFUrlKnowledgeBase` reads PDFs from URLs.
